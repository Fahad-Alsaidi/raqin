...
func testUserToManyReviewerLineRevisions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.ReviewerLineRevisions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadReviewerLineRevisions(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ReviewerLineRevisions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ReviewerLineRevisions = nil
	if err = a.L.LoadReviewerLineRevisions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ReviewerLineRevisions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpReviewerLineRevisions(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddReviewerLineRevisions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ReviewerLineRevisions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ReviewerLineRevisions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ReviewerLineRevisions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `FirstName`: `character varying`, `LastName`: `character varying`, `Email`: `character varying`, `Password`: `character varying`, `Role`: `enum.user_role('ADMIN','RAQIN','VOLUNTEER')`, `Gender`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
